 ...
 */
using namespace std;
bool Tmnper::saveIntoTmpr(string fileName, string contents, string extn) {
  if (fileName.length() == 0 || !regex_match(fileName, regex(".*\\" + extn))) return false;
 ...
 */
using namespace std;
string Tmnper::loadFromTmpr(string fileName, string extn) {
  if (fileName.length() == 0 || !regex_match(fileName, regex(".*" + extn))) return string("");
